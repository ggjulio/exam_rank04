...
void exec_pipeline(t_cmd *it)
{
	...
	status = 0;
	in = 0;
	while (it->pipe && !status)
	{
		if (pipe(fds) == -1)
			exit_fatal_error();
		pid = spawn(in, fds[1], it);
		close(fds[1]);
		if (in && it->pipe)
			close(in);
...
int main(int ac, char **av, char **env)
{
	...
	// if (cmds)
	// 	shit(cmds);
	// return 0; // avec return, sans free, le leak sanitizer est pas content, alternative, creer cmds en variable globale devrait le faire taire.
	exit(0); // avec exit, sans free, le leak sanitizer est content. Donc la moulinette aussi normalement.
}
